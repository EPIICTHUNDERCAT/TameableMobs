...
public class TamedVillagerRegistry {
	...
	public interface IVillageCreationHandler {
		/**
		 * Called when
		 * {@link net.tameablemobs.world.gen.structure.MapGenVillage} is
		 ...
		 * {@link net.tameablemobs.world.gen.structure.StructureVillagePieces}
		 *
		 * @param villagePiece
		 * @param startPiece
		 * @param pieces
	...
	private FMLControlledNamespacedRegistry<TamedVillagerProfession> professions = PersistentRegistryManager
			.createRegistry(PROFESSIONS, TamedVillagerProfession.class, null, 0, 1024, true, null, null, null, null);

	public IForgeRegistry<TamedVillagerProfession> getRegistry() {
	...
	public static class TamedVillagerCareer {
		...
		public TamedVillagerCareer addTrade(int level, ITradeList... trades) {
			...
			if (levelTrades == null) // Not sure how this could happen, but
										// screw it
			{
				levelTrades = Lists.newArrayList();
	...
	public static void onSetProfession(TameableZombie entity, ZombieType type, int network) {
		...
		if (prof == null && network != 0 || INSTANCE.professions.getId(prof) != realID) {
			throw new RuntimeException(
					"Attempted to set villager profession to unregistered profession: " + realID + " " + prof);
		}
	...
	@Deprecated
	public static TamedVillagerProfession getById(int network) {
		return INSTANCE.professions.getObjectById(network);
	}

	@Deprecated
	public static int getId(TamedVillagerProfession prof) {
		return INSTANCE.professions.getId(prof);
	}
