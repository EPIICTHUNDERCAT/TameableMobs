...
public class ModelTameablePolarBear extends ModelQuadruped {
	public ModelTameablePolarBear() {
		super(12, 0.0F);
	...
	public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
		...
		if (this.isChild) {
			...
			GlStateManager.translate(0F, -0.4219999999, 0);
			this.head.render(scale);
			this.body.render(scale);
			this.leg1.render(scale);
			this.leg2.render(scale);
	...
	public void setLivingAnimations(EntityLivingBase entitylivingbaseIn, float limbSwingAmount, float ageInTicks,
		...
		TameablePolarBear tameablePolarBear = (TameablePolarBear) entitylivingbaseIn;

		if (tameablePolarBear.isSitting()) {
			...
			leg2.offsetX = 0.1f;
			leg2.offsetY = 0f;
			leg2.rotateAngleX = ((float) Math.PI * 3F / 10F);
			...
			leg4.offsetX = 0f;
			leg4.offsetY = 0f;
			leg1.rotateAngleX = MathHelper.cos(limbSwingAmount * 0.6662F) * 1.4F * ageInTicks;
			leg2.rotateAngleX = MathHelper.cos(limbSwingAmount * 0.6662F + (float) Math.PI) * 1.4F * ageInTicks;
			leg3.rotateAngleX = MathHelper.cos(limbSwingAmount * 0.6662F + (float) Math.PI) * 1.4F * ageInTicks;
			leg4.rotateAngleX = MathHelper.cos(limbSwingAmount * 0.6662F) * 1.4F * ageInTicks;
		}
	...
	public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
		...
		float f1 = ((TameablePolarBear) entityIn).getStandingAnimationScale(f);
		f1 = f1 * f1;
		float f2 = 1.0F - f1;
