...
public class TameableGiantZombie extends EntityAnimal implements IEntityOwnable {

	private static final DataParameter<Float> DATA_HEALTH_ID = EntityDataManager
			.<Float>createKey(TameableGiantZombie.class, DataSerializers.FLOAT);
	private static final DataParameter<Boolean> BEGGING = EntityDataManager
			.<Boolean>createKey(TameableGiantZombie.class, DataSerializers.BOOLEAN);
	protected static final DataParameter<Byte> TAMED = EntityDataManager.<Byte>createKey(TameableGiantZombie.class,
			DataSerializers.BYTE);
	protected static final DataParameter<Optional<UUID>> OWNER_UNIQUE_ID = EntityDataManager
			.<Optional<UUID>>createKey(TameableGiantZombie.class, DataSerializers.OPTIONAL_UNIQUE_ID);

	protected EntityAISit aiSit;

	public TameableGiantZombie(World worldIn) {
		super(worldIn);
		setTamed(false);
	...
	protected void initEntityAI() {
		...
		aiSit = new TameableGiantZombie.EntityAISit(this);
		tasks.addTask(1, aiSit);

		tasks.addTask(5, new EntityAIFollowOwner(this, 2.0D, 5.0F, 2.0F));
		tasks.addTask(7, new EntityAIWander(this, 1.0D));
		tasks.addTask(8, new EntityAIWatchClosest(this, EntityPlayer.class, 8.0F));
		tasks.addTask(8, new TameableGiantZombie.EntityAIBeg(this, 8.0F));
		targetTasks.addTask(1, new TameableGiantZombie.EntityAIOwnerHurtByTarget(this));
		targetTasks.addTask(3, new TameableGiantZombie.AIFindPlayer(this));
	...
	public static void registerFixesSheep(DataFixer fixer) {
		EntityLiving.registerFixesMob(fixer, "TameableGiantZombie");
	}
	...
	public boolean shouldAttackEntity(EntityLivingBase p_142018_1_, EntityLivingBase p_142018_2_) {
		if (!(p_142018_1_ instanceof TameableGiantZombie) && !(p_142018_1_ instanceof EntityGhast)) {
			if (p_142018_1_ instanceof TameableGiantZombie) {
				TameableGiantZombie entityChicken = (TameableGiantZombie) p_142018_1_;
	...
	static class EntityAIBeg extends EntityAIBase {
		private final TameableGiantZombie theBat;
		private EntityPlayer thePlayer;
		private final World worldObject;
		private final float minPlayerDistance;
		private int timeoutCounter;

		public EntityAIBeg(TameableGiantZombie blaze, float minDistance) {
			theBat = blaze;
	...
	static class EntityAISit extends EntityAIBase {
		private final TameableGiantZombie theEntity;

		/** If the EntityTameable is sitting. */

		private boolean isSitting;

		public EntityAISit(TameableGiantZombie entityIn) {
			theEntity = entityIn;
			setMutexBits(5);
		}
	...
	static class EntityAIFollowOwner extends EntityAIBase {
		private final TameableGiantZombie thePet;
		...
		public EntityAIFollowOwner(TameableGiantZombie thePetIn, double followSpeedIn, float minDistIn,
				float maxDistIn) {
			thePet = thePetIn;
	...
	static class EntityAIOwnerHurtByTarget extends EntityAITarget {
		TameableGiantZombie theDefendingTameable;
		EntityLivingBase theOwnerAttacker;
		private int timestamp;

		public EntityAIOwnerHurtByTarget(TameableGiantZombie theDefendingTameableIn) {
	...
	static class AIFindPlayer extends EntityAINearestAttackableTarget<EntityPlayer> {
		private final TameableGiantZombie TameableGiantZombie;
		private EntityPlayer player;
		private int aggroTime;
		private int teleportTime;

		public AIFindPlayer(TameableGiantZombie p_i45842_1_) {
			super(p_i45842_1_, EntityPlayer.class, false);
			TameableGiantZombie = p_i45842_1_;
		}

		@Override
		public boolean shouldExecute() {
			double d0 = getTargetDistance();
			player = TameableGiantZombie.worldObj.getNearestAttackablePlayer(TameableGiantZombie.posX,
					TameableGiantZombie.posY, TameableGiantZombie.posZ, d0, d0, (Function) null,
					(@Nullable EntityPlayer player) -> (player != null)
							&& (TameableGiantZombie.shouldAttackPlayer(player)));
			return player != null;
		}
		...
		public boolean continueExecuting() {
			if (player != null) {
				if (!TameableGiantZombie.shouldAttackPlayer(player)) {
					return false;
				}
				TameableGiantZombie.faceEntity(player, 10.0F, 10.0F);
				return true;
			}
		...
		public void updateTask() {
			if (player != null) {
				...
				if (targetEntity != null) {
					if (TameableGiantZombie.shouldAttackPlayer(targetEntity)) {
						if (targetEntity.getDistanceSqToEntity(TameableGiantZombie) < 16.0D) {
						}
						teleportTime = 0;
					} else if ((targetEntity.getDistanceSqToEntity(TameableGiantZombie) > 256.0D)
							&& (teleportTime++ >= 30) && (TameableGiantZombie.teleportToEntity(targetEntity))) {
						teleportTime = 0;
					}
				}
				super.updateTask();
	...
	public static void registerFixesTameableGiantZombie(DataFixer fixer) {
		EntityLiving.registerFixesMob(fixer, "TameableGiant");
	}
	...
	@Override
	public TameableGiantZombie createChild(EntityAgeable ageable) {
		TameableGiantZombie entityTameableGiantZombie = new TameableGiantZombie(this.worldObj);
		UUID uuid = this.getOwnerId();

		if (uuid != null) {
			entityTameableGiantZombie.setOwnerId(uuid);
			entityTameableGiantZombie.setTamed(true);
		}
