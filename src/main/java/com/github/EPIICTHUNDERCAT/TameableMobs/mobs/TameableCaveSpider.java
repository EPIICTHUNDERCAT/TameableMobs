...
public class TameableCaveSpider extends EntityAnimal implements IEntityOwnable{

		 private static final DataParameter<Byte> CLIMBING = EntityDataManager.<Byte>createKey(TameableCaveSpider.class, DataSerializers.BYTE);
		...
		protected void initEntityAI() {
			...
			targetTasks.addTask(2, new TameableCaveSpider.AISpiderTarget(this, EntityPlayer.class));
	        targetTasks.addTask(3, new TameableCaveSpider.AISpiderTarget(this, EntityIronGolem.class));
			targetTasks.addTask(1, new EntityAINearestAttackableTarget(this, EntityPlayer.class, true));
			targetTasks.addTask(2, new EntityAIOwnerHurtByTarget(this));
			targetTasks.addTask(3, new TameableCaveSpider.AIFindPlayer(this));
		...
		public boolean canMateWith(EntityAnimal otherAnimal) {
			if (otherAnimal == this) {
				...
				return false;
			} else if (!(otherAnimal instanceof TameableCaveSpider)) {
				return false;
			} else {
				TameableCaveSpider entityTameableCaveSpider = (TameableCaveSpider) otherAnimal;
				return !entityTameableCaveSpider.isTamed() ? false
						: (entityTameableCaveSpider.isSitting() ? false : isInLove() && entityTameableCaveSpider.isInLove());
			}
		}
		...
		public boolean shouldAttackEntity(EntityLivingBase p_142018_1_, EntityLivingBase p_142018_2_) {
			if (!(p_142018_1_ instanceof TameableCaveSpider) && !(p_142018_1_ instanceof EntityGhast)) {
				if (p_142018_1_ instanceof TameableCaveSpider) {
					TameableCaveSpider entityChicken = (TameableCaveSpider) p_142018_1_;
		...
		static class EntityAIBeg extends EntityAIBase {
			private final TameableCaveSpider theBat;
			private EntityPlayer thePlayer;
			private final World worldObject;
			private final float minPlayerDistance;
			private int timeoutCounter;

			public EntityAIBeg(TameableCaveSpider blaze, float minDistance) {
				theBat = blaze;
		...
		static class EntityAISit extends EntityAIBase {
			private final TameableCaveSpider theEntity;

			/** If the EntityTameable is sitting. */

			private boolean isSitting;

			public EntityAISit(TameableCaveSpider entityIn) {
				theEntity = entityIn;
				setMutexBits(5);
			}
		...
		public TameableCaveSpider createChild(EntityAgeable ageable) {
			TameableCaveSpider entityTameableCaveSpider = new TameableCaveSpider(worldObj);
			UUID uuid = getOwnerId();

			if (uuid != null) {
				entityTameableCaveSpider.setOwnerId(uuid);
				entityTameableCaveSpider.setTamed(true);
			}

			return entityTameableCaveSpider;
		}

		static class EntityAIFollowOwner extends EntityAIBase {
			private final TameableCaveSpider thePet;
			...
			public EntityAIFollowOwner(TameableCaveSpider thePetIn, double followSpeedIn, float minDistIn, float maxDistIn) {
				thePet = thePetIn;
		...
		static class EntityAIOwnerHurtByTarget extends EntityAITarget {
			TameableCaveSpider theDefendingTameable;
			EntityLivingBase theOwnerAttacker;
			private int timestamp;

			public EntityAIOwnerHurtByTarget(TameableCaveSpider theDefendingTameableIn) {
		...
		static class AIFindPlayer extends EntityAINearestAttackableTarget<EntityPlayer> {
			private final TameableCaveSpider TameableCaveSpider;
			private EntityPlayer player;
			private int aggroTime;
			private int teleportTime;

			public AIFindPlayer(TameableCaveSpider p_i45842_1_) {
				super(p_i45842_1_, EntityPlayer.class, false);
				TameableCaveSpider = p_i45842_1_;
			}

			@Override
			public boolean shouldExecute() {
				double d0 = getTargetDistance();
				player = TameableCaveSpider.worldObj.getNearestAttackablePlayer(TameableCaveSpider.posX, TameableCaveSpider.posY,
						TameableCaveSpider.posZ, d0, d0, (Function) null, (@Nullable EntityPlayer player) -> (player != null)
								&& (TameableCaveSpider.shouldAttackPlayer(player)));
				return player != null;
			}
			...
			public boolean continueExecuting() {
				if (player != null) {
					if (!TameableCaveSpider.shouldAttackPlayer(player)) {
						return false;
					}
					TameableCaveSpider.faceEntity(player, 10.0F, 10.0F);
					return true;
				}
			...
			public void updateTask() {
				if (player != null) {
					...
					if (targetEntity != null) {
						if (TameableCaveSpider.shouldAttackPlayer(targetEntity)) {
							if (targetEntity.getDistanceSqToEntity(TameableCaveSpider) < 16.0D) {
							}
							teleportTime = 0;
						} else if ((targetEntity.getDistanceSqToEntity(TameableCaveSpider) > 256.0D) && (teleportTime++ >= 30)
								&& (TameableCaveSpider.teleportToEntity(targetEntity))) {
							teleportTime = 0;
						}
					}
					super.updateTask();
		...
		private boolean shouldAttackPlayer(EntityPlayer player) {
		    ...
		    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata)
		    {
		        ...
		        if (livingdata == null)
		        {
		            livingdata = new TameableCaveSpider.GroupData();
		            ...
		            {
		                ((TameableCaveSpider.GroupData)livingdata).setRandomEffect(this.worldObj.rand);
		            }
		        }

		        if (livingdata instanceof TameableCaveSpider.GroupData)
		        {
		            Potion potion = ((TameableCaveSpider.GroupData)livingdata).effect;
		    ...
		    static class AISpiderAttack extends EntityAIAttackMelee
		        {
		            public AISpiderAttack(TameableCaveSpider spider)
		            {
		    ...
		    static class AISpiderTarget<T extends EntityLivingBase> extends EntityAINearestAttackableTarget<T>
		        {
		            public AISpiderTarget(TameableCaveSpider spider, Class<T> classTarget)
		            {
